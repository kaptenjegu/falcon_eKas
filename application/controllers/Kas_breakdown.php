...
class Kas_breakdown extends CI_Controller
{
    ...
    public function laporan2()
    {
        ...
        if ($n->num_rows() > 0) {
            ...
            $ttl_saldo1 = 0;
            $ttl_saldo2 = 0;
            $table2 = '<title>KAS LUAR RAB ' . $data[0]->nama_lokasi . ' ' . $data[0]->nama_minggu . ' ' . $data[0]->nama_data_kas .  '</title>';
            $table2 .= '<table border="1" style="width: 100%;"><tr style="background-color: gray;color: white;font-weight: bold;text-align: center;"><td colspan="9">Kas Luar RAB ' . $data[0]->nama_lokasi . ' ' . $data[0]->nama_minggu . ' ' . $data[0]->nama_data_kas .  '</td></tr>';
            $table2 .= '<tr style="text-align: center;background-color: #69e842;font-weight: bold;"><td>No</td><td>Tanggal</td><td>Uraian</td><td>Qty</td><td>Harga(Rp)</td><td>Jumlah(Rp)</td><td>PIC</td><td>Nomor Kas</td></tr>';
