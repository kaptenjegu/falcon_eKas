...
class Kas_tipe extends CI_Controller
{
    ...
    {
        $data['judul'] = 'Data Kas Tipe';
        $data['page'] = 'Kas_tipe';
        $data['url'] = base_url('Kas_tipe');

        $this->db->where('tgl_delete', null);
        $this->db->order_by('tgl_add', 'desc');
        $data['kas_tipe'] = $this->db->get('fki_kas_tipe')->result();

        $this->load->view('header', $data);
        $this->load->view('kas_tipe', $data);
